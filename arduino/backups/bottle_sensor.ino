...
#define GATE_OPEN_MS    5000   // ms gate stays open before auto-close
...
void loop() {
  ...
  if (sessionActive) {
    checkSessionTimeout();
    if (!obstacleBlocked && !waitingForResult) {
      checkBottle();
    }
  }
...
void checkBottle() {
  ...
  if (present && !bottleWasPresent) {
    ...
    if (now - lastBottleTime >= BOTTLE_COOLDOWN) {
