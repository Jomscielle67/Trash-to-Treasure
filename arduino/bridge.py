...
def update_student_points(db, student_doc_id, bottles_to_add):
    ...
    try:
        ...
        # Write a deposit transaction doc so the dashboard chart has data
        write_deposit_transaction(db, student_doc_id, student_data, bottles_to_add)
        return True
...
def write_deposit_transaction(db, student_doc_id, student_data, bottles):
    """Create a deposit transaction record in Firestore for reporting/charts."""
    try:
        db.collection("transactions").add({
            "userId":      student_doc_id,
            "studentName": student_data.get("fullName") or student_data.get("name", "Unknown"),
        ...
        log("FB", "Deposit transaction written for [" + student_doc_id + "]")
    except Exception as e:
        log("FB", "Failed to write deposit transaction: " + str(e), "ERR")
...
# ============================
def send_session_notification(db, student_doc_id, bottles_deposited):
    """Write a session-end notification to Firestore for the student's dashboard."""
    if db is None:
        log("NOTIF", "Firebase offline — cannot send notification", "WARN")
        return
    if bottles_deposited == 0:
        log("NOTIF", "No bottles deposited this session — skipping notification")
        return
    try:
        pts = bottles_deposited  # 1 point per bottle
        title = "Session Complete! \U0001f389"
        body = (
            "You deposited "
            + str(bottles_deposited)
            + (" bottle" if bottles_deposited == 1 else " bottles")
            + " and earned "
            + str(pts)
            + (" point" if pts == 1 else " points")
            + " this session. Keep recycling!"
        )
        db.collection("notifications").add({
            ...
            "isRead":    False,
            "timestamp": firestore.SERVER_TIMESTAMP,
            "bottles":   bottles_deposited,
            "points":    pts,
        })
        log("NOTIF", "Sent to [" + student_doc_id + "]: "
            + str(bottles_deposited) + " bottle(s), " + str(pts) + " pt(s)")
    except Exception as e:
        log("NOTIF", "Failed to send notification: " + str(e), "ERR")
...
def main():
    ...
    session_bottles       = {}       # sid -> accepted bottle count for current session
        ...
        while True:
            ...
            if qr_port.in_waiting:
                ...
                if raw and raw not in ("READY", "HEARTBEAT") \
                        ...
                        and not raw.startswith("[METAL-RAW]") \
                        and not raw.startswith("[METAL]"):
                    log("QR-ARD", raw)

                # ── Metal sensor live logs ───────────────────────────
                if raw.startswith("[METAL] >>> DETECTED"):
                    log("METAL", ">>> OBJECT DETECTED on capacitive sensor")
                elif raw.startswith("[METAL] --- cleared"):
                    log("METAL", "--- sensor cleared")
                elif raw.startswith("[METAL-RAW]"):
                    log("METAL", raw[11:].strip())   # strip "[METAL-RAW]" prefix, keep the data
                # ────────────────────────────────────────────────────
                ...
                # Material scan result
                    ...
                    if material == "PET":
                        ...
                        sensor_port.flush()
                        if sid:
                            # Count the bottle regardless of Firebase result so the
                            # session-end notification always reflects the correct total.
                            session_bottles[sid] = session_bottles.get(sid, 0) + 1
            ...
            if sensor_port.in_waiting:
                ...
                if not raw or raw.startswith("[DIST]"):
                    ...
                    print("\n[BOTTLE] Detected for [" + sid + "] — waiting 300 ms for bottle to settle...")
                    time.sleep(0.3)   # give bottle time to rest against the cap sensor
                    print("[BOTTLE] Requesting material scan")
                    qr_port.write(b"SCAN_MATERIAL\n")
                    qr_port.flush()

                elif raw.startswith("SESSION_START:"):
                    sid = raw.split(":", 1)[1].strip()
                    session_bottles[sid] = 0
                    print("\n" + "-" * 60)
                    ...
                    sid = raw.split(":", 1)[1].strip()
                    pending_student = None
                    count = session_bottles.pop(sid, 0)
                    send_session_notification(db, sid, count)
                    print("\n" + "-" * 60)
                    print("[SESSION] ENDED    [" + sid + "]  (" + str(count) + " bottle(s) this session)")
                    print("-" * 60 + "\n")
